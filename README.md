...
This code decomposes thermal functions into a **small number of exponential terms**, enabling faster and more stable numerical simulations (for example HEOM).
...
### Bose function - via the ring-polymer radius of gyration
\[
...
### Fermi function - via the 'Fermi pole function'
\[
...
- Multiple rational decomposition backends for the obtaining of poles:
  - `AAA` (adaptive Antoulas–Anderson)  [WORKS BEST]
  - `ESPRIT_FT`(the ESPRIT algorithm, having transformed to time domain) [LESS GOOD]
  - `AAA_BT` (balanced truncation variant) [BUGGY UNFINISHED WORK IN PROGRESS]
